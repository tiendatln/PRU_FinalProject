...
public class AnubisAI : MonoBehaviour
{
    ...
    public AnubisAnimation BossAnimation;
    ...
    void SummonMummy()
    {
        if (Time.time >= nextMummyTime) // Kiểm tra hồi chiêu
        {
            if (mummyPrefab != null && AttackPosition != null)
            {
                ...
                Vector3 summonPos = AttackPosition.transform.position + new Vector3(direction * mummySummonOffsetX, 0, 0);
                Instantiate(mummyPrefab, summonPos, mummyPrefab.transform.localRotation);
